...
from .geometry import line_intersects_sphere, point_inside_cone, point_inside_cone_audacy
...
class AccessAnalysis(Analysis):
    ...
    def find_accesses(self, tof):
        ...
        for i in [i for i, x in enumerate(los) if x]:
            strand_name = "%d to %d" % (self._obj_a_slice.start, self._obj_b_slice.start + i)
            ai = AccessInstant(self.r_a[0], self.rr_b[i], tof, strand_name)
