...
from poliastro.bodies import Body, _Earth
from poliastro.frames import Planes
from poliastro.twobody import Orbit
from poliastro.twobody.propagation import *
from scipy.constants import kilo

from .propagation_bulk import markley_bulk
from .util import calc_lmn, coe2xyz_fast, hex2rgb
...
class CelestialBody(Body, ScenarioObject, ABC):
    ...
    def __init__(
        ...
        super().__init__(self, parent, k, name, symbol, R, R_polar, R_mean)
    ...
    # TODO: do something with the resulting mesh
    def draw(self, figure):
        u = np.linspace(0, 2 * np.pi, 100)
        v = np.linspace(0, np.pi, 50)
        cx = self.R_mean * np.outer(np.cos(u), np.sin(v))
        cy = self.R_mean * np.outer(np.sin(u), np.sin(v))
        cz = self.R_mean * np.outer(np.ones(np.size(u)), np.cos(v))

        # ToDo use better unit conversion.
        cx, cy, cz = cx / kilo, cy / kilo, cz / kilo

        l = mlab.mesh(cx, cy, cz, color=hex2rgb(self.plot_color), figure=figure)

    def draw_update(self, figure):
        pass


class _EarthObject(_Earth, CelestialBody):
    plot_color = '#0e4a5b'

    def __init__(self):
        super(_Earth).__init__()
